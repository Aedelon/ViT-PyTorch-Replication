...
# CLASSES -------------------------------------------------
class ViTPretrainingDataset(Dataset):
    """Configured dataset for pretraining."""
    def __init__(self, targ_dir: str, patch_size: int = 16, transform: transforms = None) -> None:
    ...
    def get_3_bit_mean_color(self, image: torch.Tensor) -> torch.Tensor:
        """Get an opened image and returns the 3 bit mean color for each patch"""
        image_size = image.shape[1]

        result = torch.Tensor()

        assert image.shape[2] == image_size and image_size % self.patch_size == 0, f"Image size must be \
                    divisible by patch size. Image height: {image_size}. Image width: {image.shape[2]} \
                    Patch size: {self.patch_size}"

        for i, patch_height in enumerate(range(0, image_size, self.patch_size)):
            for j, patch_width in enumerate(range(0, image_size, self.patch_size)):
                result = torch.cat(
                    (
                        result,
                        torch.mean(
                            image[:, patch_height:patch_height + self.patch_size,
                            patch_width:patch_width + self.patch_size],
                            dim=(1, 2),
                            keepdim=True
                        )
                    ),
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        Y = self.get_3_bit_mean_color(transformed_img)
...
def create_pretrain_dataloaders(train_dir: str,
    ...
    :param transform: torchvision transforms to perform on training and testing data.
    :param batch_size: Number of samples per batch in each of the DataLoaders.
    :param patch_size: The height/width of the patches.
    :param num_workers: An integer for number of workers per DataLoader.
    :return: A tuple of (train_dataloader, test_dataloader, class_names).
        Where class_names is a list of the target classes.
        Example usage:
    ...
    """
    # Use ImageFolder to create datasets
    train_data = ViTPretrainingDataset(train_dir, transform=transform, patch_size=patch_size)
    ...
    while current_batch_iteration < max_batch_iterations:
        for batch, (X, y) in enumerate(train_dataloader):
            ...
            if current_batch_iteration % nb_iterations_checkpoint == 0:
                ...
                }, Path(save_path) / f"vit_pretraining_checkpoint_{current_batch_iteration}.pt")
                logging.info(f"Checkpoint saved to {save_path} as "
                             f"vit_pretraining_checkpoint_{current_batch_iteration}.pt")

    # Save the whole model
    logging.info(f"Save whole model to {save_path}...")
    torch.save(model, Path(save_path) / f"vit_pretrained_model_{datetime.now().strftime('%m-%d-%Y_%H-%M-%S')}.pt")
